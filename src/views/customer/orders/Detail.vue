    ...
    <div class="px-4 py-4 mx-auto space-y-3 max-w-7xl">
      ...
      <template v-else>
        ...
        <template v-if="isJasaOrder">
          ...
          <div
            v-if="
              ['ditolak', 'dibatalkan', 'cancelled', 'rejected'].includes(
                order.status,
              )
            "
          ...
          >
            ...
            <div>
              ...
              <p
                v-if="order.booking_note && order.booking_note !== '-'"
          ...
          <div
            ...
            "
            class="p-4 bg-white border border-gray-200 rounded-2xl"
          >
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center shrink-0"
                >
            ...
            <div
              ...
              "
              class="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl flex gap-2.5 items-start"
            >
              <i
                class="pi pi-exclamation-circle text-amber-600 mt-0.5 shrink-0"
              ></i>
              <div class="text-xs text-amber-800 font-medium leading-relaxed">
                Pesanan sudah dikonfirmasi merchant. Silakan lakukan pembayaran
                agar layanan dapat diproses.
              </div>
            </div>
          </div>
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2 text-xs text-orange-700">
                ...
                <span>Menunggu konfirmasi merchant</span>
              </div>
              <span class="text-sm font-bold text-orange-700 font-mono">{{
                merchantDeadlineRemaining
              }}</span>
            </div>
            <p class="mt-1 text-xs text-orange-500">
              Merchant wajib merespon sebelum batas waktu berakhir. Jika
              terlewati, pesanan akan otomatis dibatalkan.
            </p>
          </div>
          ...
          <div
            v-if="
              order.status === 'menunggu_selesai' ||
              order.status === 'menunggu_konfirmasi_selesai'
            "
            class="p-3 bg-orange-50 border border-orange-200 rounded-2xl"
          >
            <div class="flex items-center justify-between">
              ...
              <span class="text-sm font-bold text-orange-700 font-mono">{{
                completionDeadlineRemaining
              }}</span>
            </div>
            <p class="mt-1 text-xs text-orange-500">
              Jika tidak dikonfirmasi dalam 24 jam, pesanan akan otomatis
              selesai.
            </p>
          </div>
          ...
          >
            ...
            <div>
              ...
              <p class="text-xs text-red-500">
                Batas waktu respon merchant telah berakhir.
              </p>
            </div>
          </div>
          ...
          <div class="p-4 bg-white border border-gray-200 rounded-2xl">
            <div class="flex items-start gap-3">
              <div
                class="w-10 h-10 overflow-hidden bg-gray-200 rounded-full shrink-0 flex items-center justify-center"
              >
                <img
                  v-if="order.merchant?.logo_url || order.merchant?.logoUrl"
                  :src="
                    resolveImageUrl(
                      order.merchant?.logo_url || order.merchant?.logoUrl,
                    )
                  "
          ...
          >
            ...
            <div class="px-4 py-3 space-y-4">
              <div class="flex gap-3">
                <div
                  class="w-12 h-12 overflow-hidden bg-gray-200 rounded-xl shrink-0 flex items-center justify-center"
                >
                  <img
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="mt-1 text-xs text-muted-foreground">
                    Kategori: {{ order.category_name || "Layanan Jasa" }}
                  </div>
              ...
              <div class="pt-3 space-y-2 border-t border-gray-200">
                ...
                >
                  ...
                  <div class="text-green-600 font-semibold">
                    -Rp {{ formatIDR(order.discount_total) }}
                  </div>
                </div>
                <div
                  v-if="!order.is_cod && order.payment_fee > 0"
                  class="flex items-center justify-between text-xs"
                >
                  <div class="text-muted-foreground">Biaya Pembayaran</div>
                  <div class="text-muted-foreground">
                    Rp {{ formatIDR(order.payment_fee) }}
                  </div>
                </div>
                <div class="flex items-end justify-between pt-2">
                  <div class="text-xs text-muted-foreground"></div>
                  <div class="text-xl font-extrabold text-black">
                    Rp {{ formatIDR(order.total_payment) }}
                  </div>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div class="px-4 py-3 space-y-3">
              ...
              <div
                v-if="order.display_address && order.display_address !== '-'"
              ...
              >
                ...
                <div
                  class="text-xs font-semibold text-black text-right max-w-[60%]"
                >
                  {{ order.display_address }}
                </div>
              </div>
              <div
                v-if="order.date_formatted || order.booking_time"
          ...
          >
            ...
            >
              <p class="text-xs text-purple-600">
                ...
                {{
                  ...
                  order.jasa_order_item?.completion_note
                }}
              </p>
            </div>
            <div
            ...
            <div
              v-else
              class="text-center py-4 text-gray-500 text-sm bg-gray-50 rounded-xl border border-gray-100"
            >
          ...
          <div class="py-3 mx-auto max-w-7xl">
            ...
            >
              ...
              <div
                v-if="order.review.is_anonymous"
                class="text-xs text-gray-500 mb-2"
              >
                Ditampilkan sebagai Anonim
              </div>
              <div
                v-if="order.review.created_at"
            ...
            >
              ...
              <p
                v-if="order.review.merchant_reply_at"
                class="text-xs text-gray-400 mt-2 pl-10"
              >
                {{ formatDate(order.review.merchant_reply_at) }}
                {{ formatTime(order.review.merchant_reply_at) }}
              </p>
            </div>
          </div>
        </template>
        ...
        <template v-else>
          ...
          <div
            v-if="
              ['cancelled', 'rejected', 'undelivered'].includes(order.status)
            "
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p
                  v-if="order.meta.note || order.meta.failed_reason"
            ...
            >
              <div v-if="isConfirmExpired" class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-red-700"
                  >Batas waktu konfirmasi UMKM habis. Pesanan akan
                  dibatalkan.</span
                >
              </div>
              <div v-else class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-amber-700"
                    >Menunggu konfirmasi UMKM</span
                  >
                </div>
                <span class="text-sm font-bold text-amber-700 font-mono">{{
                  confirmCountdownText
                }}</span>
              </div>
            </div>
          ...
          <div class="p-4 bg-white border border-gray-200 rounded-2xl">
            <div class="space-y-4">
              <template v-if="order.meta.delivery_type === 'pickup'">
                <div class="flex items-start gap-3">
                  <div
                    class="w-10 h-10 mt-1 overflow-hidden bg-gray-200 rounded-full shrink-0 flex items-center justify-center"
                  >
                    <img
                      v-if="order.pickup.logoUrl"
                      :src="order.pickup.logoUrl"
                      class="object-cover w-full h-full"
                      alt="Store logo"
          ...
          <div
            v-if="order.meta.proof_image_url"
            class="overflow-hidden bg-white border border-gray-200 rounded-2xl mb-4"
          >
            <div class="px-4 py-3 border-b border-gray-200">
              <div class="text-base font-extrabold text-black">
            ...
            <div class="p-4 flex justify-center">
              <img
                :src="order.meta.proof_image_url"
          ...
          <div class="py-3 mx-auto max-w-7xl">
            <div
              v-if="order?.status === 'pending' && isPaymentExpired"
              class="mb-3 text-center text-sm font-semibold text-red-600 bg-red-50 py-2 rounded-xl"
            >
              Waktu pembayaran telah habis.
            </div>
            <div
              v-else-if="order?.status === 'pending' && countdownText"
              class="mb-3 text-center text-sm font-medium text-amber-700 bg-amber-50 py-2 rounded-xl"
            >
            ...
            <Button
              v-if="
                ...
                order?.meta?.payment_method?.toUpperCase() !== 'COD'
              "
              block
              :loading="completing"
            ...
            <template v-if="order?.status === 'completed'">
              ...
              <Button
                v-if="
                  ...
                  order.items[0]?.can_update_review
                "
                block
                @click="
                  $router.push({
                    path: `/review/product/${order.id}/${order.items[0]?.productId}`,
                    query: {
...
const merchantDeadlineRemaining = computed(() => {
  ...
  if (
    ...
    o.status !== "menunggu_konfirmasi_merchant"
  )
    return null;
...
const completionDeadlineRemaining = computed(() => {
  ...
  if (!o) return null;
  if (
    o.status !== "menunggu_selesai" &&
    o.status !== "menunggu_konfirmasi_selesai"
  )
    return null;
...
const STATUS_MAP = {
  pending: 0,
  menunggu_konfirmasi_merchant: 0,
  diterima: 1,
  accepted: 1,
  responsed: 1,
  layanan_dikerjakan: 2,
  dikerjakan: 2,
  processing: 2,
  in_progress: 2,
  menunggu_konfirmasi_selesai: 3,
  menunggu_selesai: 3,
  selesai: 4,
  completed: 4,
  ditolak: -1,
...
const jasaOrder = computed(() => {
  ...
  const rawStatus = String(
    o.status || o.service_status || o.order_status || "",
  ).toLowerCase();
  ...
  const serviceTypeLabel =
    serviceTypeMap[o.service_type] || o.service_type || "";
  ...
  const bookingTypeLabel =
    bookingTypeMap[o.mekanisme_pemesanan] || o.mekanisme_pemesanan || "";

  const dateFormatted = o.booking_date
    ? new Date(o.booking_date).toLocaleDateString("id-ID", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      })
  ...
  const statusLabelMap = {
    menunggu_konfirmasi: "Menunggu Konfirmasi Merchant",
    menunggu_konfirmasi_merchant: "Menunggu Konfirmasi Merchant",
    pending: "Menunggu Konfirmasi Merchant",

    diterima:
      isPaid || isOrderCod ? "Pembayaran Berhasil" : "Pesanan Dikonfirmasi",
    accepted: "Pesanan Dikonfirmasi",
    responsed: "Pesanan Dikonfirmasi",

    layanan_dikerjakan: "Layanan Diproses",
    dikerjakan: "Layanan Diproses",
    processing: "Layanan Diproses",

    menunggu_konfirmasi_selesai: "Menunggu Persetujuan Customer",
    menunggu_selesai: "Menunggu Persetujuan Customer",
  ...
  let statusLabel =
    statusLabelMap[rawStatus] || o.status_label || rawStatus.replace(/_/g, " ");

  // Flow jasa baru:
  // Status diterima punya arti berbeda tergantung pembayaran.
  if (rawStatus === "diterima" && !isPaid && !isOrderCod) {
  ...
  // 6-step tracking stepper for Jasa — flow aman: merchant confirm dulu, baru bayar.
  let activeStepKey = "menunggu_konfirmasi_merchant";

  if (
    ["menunggu_konfirmasi", "menunggu_konfirmasi_merchant", "pending"].includes(
      rawStatus,
    )
  ) {
    activeStepKey = "menunggu_konfirmasi_merchant";
  } else if (rawStatus === "diterima" && !isPaid && !isOrderCod) {
  ...
  const tracking = [
    {
      ...
      done: !isCanceledOrExpired && activeIndex >= 0,
    },
    {
      key: "pesanan_dikonfirmasi",
      icon: "pi-check-circle",
      label: "Pesanan Dikonfirmasi",
      done: !isCanceledOrExpired && activeIndex >= 1,
    },
    {
      key: "pembayaran_berhasil",
      icon: "pi-wallet",
      label: "Pembayaran Berhasil",
      done: !isCanceledOrExpired && activeIndex >= 2,
    },
    {
      key: "layanan_diproses",
      icon: "pi-cog",
      label: "Layanan Diproses",
      done: !isCanceledOrExpired && activeIndex >= 3,
    },
    {
      key: "menunggu_persetujuan_customer",
      icon: "pi-check-circle",
      label: "Menunggu Persetujuan",
      done: !isCanceledOrExpired && activeIndex >= 4,
    },
    {
      key: "selesai",
      icon: "pi-home",
      label: "Selesai",
      done: !isCanceledOrExpired && activeIndex >= 5,
    },
  ];
  ...
  const isOrderStatus =
    orderStatusKeys.includes(rawStatus) ||
    rawStatus.startsWith("diterima") ||
    rawStatus.startsWith("selesai");

  const statusVariant = isOrderStatus ? "order" : isPaid ? "order" : "payment";
  const statusLabelStatus =
    !isOrderStatus && !isPaid ? "pending" : isPaid ? "completed" : rawStatus;
  ...
  return {
    ...
    id: o.id,
    order_number:
      o.invoice || o.order_number || `ORD-${String(o.id).padStart(6, "0")}`,
    jasa_order_item_id: o.jasa_order_item_id || null,
    is_reviewed:
      o.is_reviewed ||
      o.jasa_order_item?.is_reviewed ||
      o.jasaOrderItem?.isReviewed ||
      false,
    can_review:
      o.can_review !== undefined
        ? o.can_review
        : rawStatus === "selesai" &&
          !(o.is_reviewed || o.jasa_order_item?.is_reviewed),
    can_update_review:
      o.can_update_review !== undefined
        ? o.can_update_review
        : o.is_reviewed && (o.review?.update_count ?? 0) < 1,
    ...
    merchant: {
      ...
      name: o.merchant?.name || o.merchant_name || "UMKM",

      phone: o.merchant?.phone || o.merchant_phone || null,

      address:
        o.merchant?.address || o.merchant_address || o.umkm_address || null,

      primary_address:
        o.merchant?.primary_address || o.merchant?.primaryAddress || null,

      logo_url:
        o.merchant?.logo_url ||
        o.merchant?.logoUrl ||
        o.merchant?.logo ||
        o.merchant?.logo_path ||
        o.merchant?.image ||
        o.merchant_logo_url ||
        o.merchant_logo ||
        o.logo_url ||
    ...
    subtotal: Number(o.subtotal ?? o.amounts?.subtotal ?? o.total_price ?? 0),
    platform_fee: Number(o.platform_fee ?? o.admin_fee ?? o.amounts?.platform_fee ?? 0),
    payment_fee: Number(o.payment_fee ?? o.payment?.payment_fee ?? o.amounts?.platform_fee ?? 0),
    ...
    voucher_name:
      ...
      null,
    is_cod: isOrderCod,
    total_payment: Number(o.total_payment ?? o.amounts?.total ?? o.total_price ?? 0),
    payment_status_display: isOrderCod
      ? ["selesai", "ditolak", "dibatalkan", "expired"].includes(rawStatus)
        ? "Dibayar di Tempat"
        : "COD - Menunggu Bayar"
    ...
    review:
      o.review || o.jasa_order_item?.review || o.jasaOrderItem?.review || null,
    completion_note:
      o.completion_note ||
...
const productOrder = computed(() => {
  ...
  const merchantAddressObj =
    o.merchant?.primary_address || o.merchant?.primaryAddress;
...
const statusLabelStatus = computed(
  () => order.value?.statusLabelStatus || "pending",
);
...
function getProductImage(item) {
  ...
  const directImage =
    item.image || item.product_image || item.photo || item.thumbnail;
...
function goToReview() {
  ...
  if (!numericOrderId || !jasaItemId) {
    console.warn("[Detail] Cannot review: missing ids:", {
      numericOrderId,
      jasaItemId,
    });
...
async function copyOrderCode() {
  try {
    const orderCode =
      order.value?.meta?.order_code || order.value?.order_number || "";
...
function needsPayment() {
  ...
  // Flow jasa baru:
  // Customer hanya boleh membayar setelah merchant menerima pesanan.
  if (rawStatus !== "diterima") return false;

  const ps = String(o.payment_status || "").toUpperCase();
  if (["PAID", "SETTLED", "SUCCEEDED", "SUDAH_BAYAR", "LUNAS"].includes(ps)) {
    return false;
  }
...
async function retryPayment() {
  ...
  try {
    const existingInvoiceUrl =
      o.invoice_url || o.payment?.invoice_url || o.xendit_invoice_url;
    ...
    toast.error(
      err.response?.data?.message || "Gagal membuat invoice pembayaran.",
    );
  }
}
...
async function verifyPaymentFromXendit(retryCount = 0) {
  ...
  try {
    ...
    if (retryCount < MAX_RETRY) {
      toast.info(
        `Mengkonfirmasi pembayaran... (${retryCount + 1}/${MAX_RETRY})`,
      );
      setTimeout(() => {
    ...
    if (retryCount < MAX_RETRY) {
      ...
      toast.error(
        "Gagal memverifikasi pembayaran. Hubungi penjual jika sudah bayar.",
      );
    }
  } finally {
