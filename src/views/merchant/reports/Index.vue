<template>
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="flex items-center justify-between gap-3 p-4 bg-white border border-gray-200 shadow-sm rounded-2xl">
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            class="flex items-center justify-center w-10 h-10 transition bg-white border border-gray-300 rounded-xl hover:bg-gray-50 shrink-0"
          >
      ...
      >
        ...
        <template #cell-customer_name="{ item }">
          <div class="space-y-0.5">
            ...
            <div v-if="item.customer_phone" class="text-xs text-gray-500 flex items-center gap-1">
    ...
    <div class="sm:hidden space-y-3">
      <div v-if="loading" v-for="n in 3" :key="n" class="p-4 bg-white border border-gray-100 shadow-sm rounded-2xl animate-pulse">
        <div class="h-4 bg-gray-200 rounded w-1/3 mb-2"></div>
        <div class="h-3 bg-gray-200 rounded w-1/4"></div>
      </div>
      <div v-else-if="transactions.length === 0" class="p-8 text-center bg-white border border-gray-100 shadow-sm rounded-2xl">
      ...
      <div
        ...
        :key="order.id"
        class="p-4 bg-white border border-gray-200/80 shadow-sm rounded-2xl space-y-3 transition-all hover:border-gray-300"
      >
        ...
        <!-- Middle: Buyer info + Delivery badge -->
        <div class="flex items-center justify-between py-1 text-xs">
          ...
          <div class="flex items-center gap-1.5">
            <span
              ...
              class="px-2 py-0.5 text-[10px] font-medium bg-gray-100 text-gray-700 rounded-md"
            >
              {{ order.payment_method }}
            </span>
            <span
              v-if="!isFailedOrCancelled(order.status) && order.delivery_type && order.delivery_type !== '-'"
              class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium rounded-md"
              :class="order.delivery_type === 'Ambil Sendiri' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'"
            >
        ...
        <!-- Bottom: Financial calculation -->
        <div class="pt-2 border-t border-gray-100 flex items-center justify-between text-xs">
          <div class="text-gray-500">
            <span>Gross: </span>
            <span class="font-medium text-gray-700">Rp {{ formatIDR(order.gross_amount) }}</span>
          </div>
          <div class="text-right">
            <span class="text-[10px] text-gray-400 block">Pendapatan Bersih</span>
            <span
              class="text-sm font-bold"
